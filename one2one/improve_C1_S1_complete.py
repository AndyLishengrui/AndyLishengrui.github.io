...
def get_bible_verse(book, chapter, verse_start, verse_end=None, version='cuv'):
    """
    从Bible API获取经文
    book: 书卷名（英文，如 'Isaiah'）
    chapter: 章
    verse_start: 起始节
    verse_end: 结束节（可选）
    version: 版本（cuv=和合本, rcuv=新译本）
    """
    book_mapping = {
        '以赛亚书': 'Isaiah',
        '罗马书': 'Romans',
    ...
    eng_book = book_mapping.get(book, book)
    
    if verse_end and verse_end != verse_start:
        verses = f"{verse_start}-{verse_end}"
    else:
        verses = str(verse_start)
    
    url = f"https://bible-api.com/{eng_book}+{chapter}:{verses}?translation={version}"
...
# 获取和合本经文
print("📖 正在获取和合本经文...")
cuv_text = get_bible_verse('以赛亚书', 59, 1, 2, 'cuv')

if not cuv_text:
    # 如果API失败，使用备用经文
    cuv_text = "看哪，耶和华的膀臂并非缩短，不能拯救，耳朵并非发沉，不能听见，但你们的罪孽使你们与神隔绝；你们的罪恶使他掩面不听你们。"
    print("⚠️  使用备用和合本经文")
else:
    print(f"✅ 获取成功: {cuv_text[:50]}...")

# 新译本经文（从原文档提取）
rcuv_text = "看哪！耶和华的手不是缩短了，以致不能拯救；他的耳朵不是不灵，不能听见；而是你们的罪孽使你们与你们的神隔绝；你们的罪恶使他掩面不顾你们，不听你们的祷告。"
...
answer_data = {
    ...
    "answers": {
        "q1_以赛亚书 59:1,2": {
            ...
            "text": cuv_text,  # 和合本作为默认答案
            "version": "和合本",
            "version_alt": "新译本",
            "text_alt": rcuv_text,  # 新译本作为参考
            "has_data": True
        }
    }
}
    ...
    <div class="container">
        ...
        <div class="action-bar">
            <div class="score-display">
                已完成: <span class="score-number" id="scoreDisplay">0/1</span>
            </div>
            <div>
    ...
    <script>
        ...
        async function submitAnswers() {
            ...
            scoreDetailsEl.innerHTML = `
                ...
                <p>🎯 成绩计算: 基于答案框正确率 (${totalInputs > 0 ? Math.round((correctInputs/totalInputs)*100) : 0}% 完全正确)</p>
            `;
...
print("\n改进内容：")
print("1. ✅ 和合本作为默认答案")
print("2. ✅ 显示多版本参考答案（和合本 + 新译本）")
print("3. ✅ 修复成绩统计，添加详细评分页面")
print("4. ✅ 调大字体，统一行高")
